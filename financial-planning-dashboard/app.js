...
const App = (function () {
  // --- Chart Instances ---
    ...
    const fmtCurrencySimple = new Intl.NumberFormat("en-US", {
      style: "currency",
      currency: "USD",
    ...
    const elements = {
      ...
      invPerfSelect: document.getElementById("invPerfSelect"),
      formLogInvHistory: document.getElementById("formLogInvHistory"),
      logInvDate: document.getElementById("logInvDate"),
      logInvBalance: document.getElementById("logInvBalance"),
      chartInvPerformance: document.getElementById("chartInvPerformance"),
      tbInvPerformance: document.getElementById("tbInvPerformance"),
    };
    ...
    const state = {
      ...
      investmentHistory: [], // Aggregated per-account history
      paystubs: [],
      settings: {}, // Store global settings response
    ...
    elements.tabButtons.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (targetId === "tab-investment-performance") {
          populateInvPerfSelect();
          fetchInvestmentHistoryFromAPI();
        }
    ...
    function updateHomeScreenMetrics() {
      ...
      if (hourlyEl) {
        ...
        const latestPs = state.dataPaystubs[0]; // Sorted descending in fetchPaystubsFromAPI
        if (latestPs) {
    ...
    window.calcWealthActuals = function () {
      try {
        ...
        // 1. Windowing (Strict 50/50 split)
        let minDate, maxDate;
        if (timeframe === "daily") {
        ...
        // 2. Data Preparation
        const invHistory = [...(state.investmentHistory || [])];
        const paystubs = [...(state.dataPaystubs || [])];
        const valuations = [];
        Object.keys(state.valuations || {}).forEach(k => valuations.push(...state.valuations[k]));
        const loanPayments = [];
        Object.keys(state.loanPayments || {}).forEach(k => loanPayments.push(...state.loanPayments[k]));

        const allActualDates = new Set();
        invHistory.forEach(ih => allActualDates.add(ih.record_date));
        valuations.forEach(v => allActualDates.add(v.valuation_date));
        paystubs.forEach(p => allActualDates.add(p.pay_date));
        loanPayments.forEach(lp => allActualDates.add(lp.payment_date));
        // Add "today" to ensure we have a current point
        allActualDates.add(now.toISOString().split("T")[0]);

        const sortedActualDates = Array.from(allActualDates)
          .filter(d => new Date(d) >= minDate && new Date(d) <= now)
          .sort();

        // 3. Projections Calibration
        const projections = [];
        const startYear = now.getFullYear();
        const projectionsData = state.dataNetWorth || [];

        // Calculate Anchor (Current Actual NW)
        let currentActualNW = 0;
        (state.investments || []).forEach(inv => currentActualNW += (inv.current_value || 0));
        (state.properties || []).forEach(prop => {
          const vals = state.valuations[prop.id] || [];
          const latestV = (vals.length > 0) ? [...vals].sort((a,b)=>new Date(b.valuation_date)-new Date(a.valuation_date)).find(v => new Date(v.valuation_date) <= now) : null;
          const val = latestV ? latestV.estimated_value : (prop.purchase_price || 0);
          
          const pmts = state.loanPayments[prop.id] || [];
          const principalPaid = pmts.filter(p => new Date(p.payment_date) <= now).reduce((sum, p) => sum + (p.principal_amount || 0) + (p.overage_amount || 0), 0);
          const remainingLoan = Math.max(0, (prop.propInitLoan || 0) - principalPaid);
          
          currentActualNW += (val - remainingLoan);
        });

        if (projectionsData.length > 0) {
          projections.push({ date: new Date(now), netWorth: currentActualNW });
        ...
        // --- Pre-fill Keys to enforce 50/50 split on X-axis ---
        let fillDay = new Date(minDate);
        let iterations_fill = 0;
        while (fillDay <= maxDate && iterations_fill < 1000) {
          const k = getPeriodKey(fillDay, timeframe);
          if (!grouped[k]) grouped[k] = { actualNW: null, projNW: null, date: new Date(fillDay) };
          
          if (timeframe === "daily") fillDay.setDate(fillDay.getDate() + 1);
          else if (timeframe === "weekly") fillDay.setDate(fillDay.getDate() + 7);
          else if (timeframe === "monthly") fillDay.setMonth(fillDay.getMonth() + 1);
          else if (timeframe === "quarterly") fillDay.setMonth(fillDay.getMonth() + 3);
          else fillDay.setFullYear(fillDay.getFullYear() + 1);
          iterations_fill++;
        }
        ...
        sortedActualDates.forEach(dateStr => {
          ...
          // Investments
          (state.investments || []).forEach(inv => {
            const hist = (state.investmentHistory || [])
              .filter(ih => ih.investment_id === inv.id && ih.record_date <= dateStr)
              .sort((a, b) => new Date(b.record_date) - new Date(a.record_date));
            nw += (hist.length > 0) ? hist[0].balance : (inv.current_value || 0);
          });
          // Properties (Equity = Value - (InitLoan - PrincipalPaid))
          (state.properties || []).forEach(prop => {
            const propVals = state.valuations[prop.id] || [];
            const latestV = [...propVals].sort((a,b)=>new Date(b.valuation_date)-new Date(a.valuation_date)).find(v => v.valuation_date <= dateStr);
            const val = latestV ? latestV.estimated_value : (prop.purchase_price || 0);

            const pmts = state.loanPayments[prop.id] || [];
            const principalPaid = pmts.filter(p => p.payment_date <= dateStr).reduce((sum, p) => sum + (p.principal_amount || 0) + (p.overage_amount || 0), 0);
            const remainingLoan = Math.max(0, (prop.propInitLoan || 0) - principalPaid);
            nw += (val - remainingLoan);
          });

          if (grouped[key]) {
            if (grouped[key].actualNW === null || d >= grouped[key].date) {
               grouped[key].actualNW = nw;
               grouped[key].date = d;
            }
          }
        });
        ...
        if (projections.length > 0) {
          ...
          while (currDay <= maxProjectionDate && iterations_proj < 1000) {
            ...
            if (currDay >= minDate && currDay <= maxDate) {
              ...
              let interceptedNW = 0;
              if (idx <= 0) interceptedNW = projections[0].netWorth;
              else {
                const p1 = projections[idx-1], p2 = projections[idx];
                const t = (currDay - p1.date) / (p2.date - p1.date);
                interceptedNW = p1.netWorth + (p2.netWorth - p1.netWorth) * t;
              }
              if (grouped[key]) grouped[key].projNW = interceptedNW;
            }
        ...
        // Ensure the bounds have at least a 0 value to force Chart.js to render the full X-axis range
        if (finalKeys.length > 0) {
          const firstK = finalKeys[0];
          const lastK = finalKeys[finalKeys.length - 1];
          if (grouped[firstK].actualNW === null) grouped[firstK].actualNW = 0;
          if (grouped[lastK].projNW === null) {
             // Fallback to the latest projection point if possible
             const projKeys = finalKeys.filter(k => grouped[k].projNW !== null);
             if (projKeys.length > 0) grouped[lastK].projNW = grouped[projKeys[projKeys.length-1]].projNW;
             else grouped[lastK].projNW = currentActualNW;
          }
        }

        state.wealthActualsData = finalKeys.map(k => ({ label: k, ...grouped[k] }));
        console.log("calcWealthActuals Debug:", { 
          timeframe, 
        ...
        console.error("calcWealthActuals Error:", err);
      }
    };
        ...
        .map(
          (d) => `
            <tr class="${d.isCrossoverMarker ? "bg-brand-500/10 transition-colors" : "hover:bg-white/5 transition-colors"}">
                <td class="py-3 px-4 font-medium">${state.currentYear + d.year - 1}</td>
    ...
    const API_URL_INVESTMENT_HISTORY = "http://localhost:8000/api/investment-history";
    ...
    async function fetchPropertiesFromAPI() {
      try {
        ...
        if (resp.ok) {
          ...
          if (!state.isInitialLoad) runAllCalculations();
        }
      } catch (e) { console.error("Properties fetch error:", e); }
    }
  ...
  async function fetchInvestmentHistoryFromAPI() {
    try {
      const resp = await fetch(API_URL_INVESTMENT_HISTORY + "?user_id=1");
      if (resp.ok) {
        state.investmentHistory = await resp.json();
        populateInvPerfSelect();
        if (!state.isInitialLoad) {
          calcInvestmentPerformance();
        }
      ...
      console.error("Failed to fetch investment history:", e);
    }
  }

  async function fetchPaystubsFromAPI() {
    try {
  ...
  function populateInvPerfSelect() {
    if (!elements.invPerfSelect) return;
    const currentVal = elements.invPerfSelect.value;
    elements.invPerfSelect.innerHTML = '<option value="all">All Accounts (Combined)</option>';
    state.investments.forEach(inv => {
      const opt = document.createElement("option");
      opt.value = inv.id;
      opt.textContent = inv.name;
      elements.invPerfSelect.appendChild(opt);
    });
    if (currentVal) elements.invPerfSelect.value = currentVal;
  }

  async function logInvestmentHistory(e) {
    if (e) e.preventDefault();
    const accountId = elements.invPerfSelect.value;
    if (accountId === "all") {
      alert("Please select a specific account to log a balance.");
      return;
    }
    const recordDate = elements.logInvDate.value;
    const balance = parseFloat(elements.logInvBalance.value.replace(/,/g, ""));
    if (isNaN(balance)) {
    ...
    try {
      const resp = await fetch(API_URL_INVESTMENT_HISTORY + "?user_id=1", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          investment_account_id: parseInt(accountId),
          record_date: recordDate,
          balance: balance
        })
      });
      if (resp.ok) {
        elements.logInvBalance.value = "";
        await fetchInvestmentHistoryFromAPI();
      } else {
        const err = await resp.json();
        alert("Error logging balance: " + (err.detail || "Unknown error"));
      }
    } catch (e) {
      console.error("Log investment history error:", e);
    }
  }

  function calcInvestmentPerformance() {
    if (!elements.invPerfSelect) return;
    const selectedId = elements.invPerfSelect.value || "all";
    let filteredHistory = [];
    let growthRate = 0;
    let startBalance = 0;
    ...
    if (selectedId === "all") {
      const dateMap = {};
      state.investmentHistory.forEach(h => {
        const d = h.record_date;
        dateMap[d] = (dateMap[d] || 0) + h.balance;
      });
      filteredHistory = Object.keys(dateMap).sort().map(d => ({ record_date: d, balance: dateMap[d] }));
      
      let totalValue = 0;
      let weightedSum = 0;
      state.investments.forEach(inv => {
        totalValue += inv.current_value;
        weightedSum += inv.current_value * (inv.growth_target / 100);
      });
      growthRate = totalValue > 0 ? (weightedSum / totalValue) : 0.07;
      startBalance = totalValue;
    } else {
      const invId = parseInt(selectedId);
      const inv = state.investments.find(i => i.id === invId);
      if (!inv) return;
      filteredHistory = state.investmentHistory.filter(h => h.investment_account_id === invId).sort((a,b) => new Date(a.record_date) - new Date(b.record_date));
      growthRate = inv.growth_target / 100;
      startBalance = inv.current_value;
    }

    const projectionYears = state.timelineYears || 25;
    const projections = [];
    let currentBal = startBalance;
    const now = new Date();
    
    for (let i = 1; i <= projectionYears; i++) {
        const yearDate = new Date(now.getFullYear() + i, now.getMonth(), now.getDate());
        const growth = currentBal * growthRate;
        const endBal = currentBal + growth;
        projections.push({
            date: yearDate,
            start: currentBal,
            growth: growth,
    ...
    renderInvestmentPerformance(filteredHistory, projections);
  }

  let chartInvPerf = null;
  function renderInvestmentPerformance(history, projections) {
    if (elements.tbInvPerformance) {
        elements.tbInvPerformance.innerHTML = projections.map(p => `
            <tr class="hover:bg-brand-500/5 transition-colors">
                <td class="py-3 px-4">${p.date.getFullYear()}</td>
                <td class="py-3 px-4 text-right">${fmtCurrency.format(p.start)}</td>
                <td class="py-3 px-4 text-right text-emerald-400">+${fmtCurrency.format(p.growth)}</td>
                <td class="py-3 px-4 text-right font-bold text-white">${fmtCurrency.format(p.end)}</td>
            </tr>
        `).join("");
    }

    if (!elements.chartInvPerformance) return;
    const ctx = elements.chartInvPerformance.getContext("2d");
    if (chartInvPerf) chartInvPerf.destroy();

    const histLabels = history.map(h => h.record_date);
    const histData = history.map(h => h.balance);
    
    const lastHist = history.length > 0 ? history[history.length - 1] : { balance: projections.length > 0 ? projections[0].start : 0, record_date: new Date().toISOString().split('T')[0] };
    
    const projLabels = projections.map(p => p.date.toISOString().split('T')[0]);
    const projData = projections.map(p => p.end);

    const totalPoints = histLabels.length + projLabels.length;
    const histDataPoints = new Array(totalPoints).fill(null);
    const projDataPoints = new Array(totalPoints).fill(null);

    // Fill historical data
    histData.forEach((val, i) => {
        histDataPoints[i] = val;
    });

    // Fill projection data matching the transition
    if (histLabels.length > 0) {
        projDataPoints[histLabels.length - 1] = lastHist.balance;
    }
    projData.forEach((val, i) => {
        projDataPoints[histLabels.length + i] = val;
    });

    chartInvPerf = new Chart(ctx, {
        type: 'line',
        data: {
        ...
        options: {
            ...
            scales: {
                y: {
                    beginAtZero: false,
                    grid: { color: 'rgba(255,255,255,0.05)' },
                    ticks: { color: '#94a3b8', callback: (val) => fmtCurrencySimple.format(val) }
                },
                x: {
                    grid: { display: false },
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    borderWidth: 1,
                    callbacks: {
                        label: (ctx) => `${ctx.dataset.label}: ${fmtCurrency.format(ctx.raw)}`
                    }
                }
            }
        }
  ...
  if (elements.invPerfSelect) {
    elements.invPerfSelect.addEventListener("change", calcInvestmentPerformance);
  }
  if (elements.formLogInvHistory) {
    elements.formLogInvHistory.addEventListener("submit", logInvestmentHistory);
  }
